...
vector< string > pipeVector(11);
vector< int > pids(11);


void comunicadora(string nom,  datosEnvio dE){
								int pipeC;
								cout<<"|--->ZONA DE ESCRITURA A PIPES			                               |\n";

								if ((pipeC = open(nom.c_str(), O_WRONLY|O_NONBLOCK)) == -1) {
								...
								cout<<"|--->El gestor enviando respuesta a traves de: "<<nom<<"	                       |\n";
...
void datosEntrada( datosEnvio opcion, vector<datosCliente>  &dc, string mode, set< int > &conectados)
{
								...
								case 0:
								{
																pipeVector[opcion.id]="nan";
																pids[opcion.id]=0;
																conectados.erase(opcion.id);
								} break;
								case 1:
								{
																// Follow-------------------------
																...
																{
																								cout<<"|  2. Validando si se intenta seguir a si mismo								|\n";
																								if(opcion.id == targetid) {
																																dE.action = 4;
																								}else{
																																cout<<"|  3. Validando si no se esta siguiendo ya al usuario					|\n";
																...
																cout<<"|  4. Enviando respuesta al cliente...						|\n";
								...
								case 2:
								{
																...
																if(targetid<1 || targetid>10)
																{
																								...
																								cout<<"|  2. Validando si se intenta dejar de seguir a si mismo								|\n";
																								if(opcion.id == targetid) {
																																dE.action = 4;
																								}else{
																																cout<<"|  3. Validando si se esta siguiendo ya al usuario					|\n";
																																if(dc[opcion.id].follow.find(targetid) != dc[opcion.id].follow.end()) {
								...
								case 3:
								{
																// Recibir Tu&T------------------------
																string message;
																cout<<"| Gestor recibiendo Tu&T del cliente: "<<opcion.id<<"													|\n";
																cout<<"|   1. El mensaje recibido es: "<<opcion.message<<"															|"<<endl;
																...
																if(message.length() <= 140)
																{
																								...
																								cout<<"|     El Tu&T se le ha añadido al cliente: "<<opcion.id<<"												|"<<endl;
																								dc[opcion.id].tweet.push_back(message);
																								cout<<"|     Numero de Tu&T's que posee el cliente "<<opcion.id<<": "<<dc[opcion.id].tweet.size()<<"				|"<<endl;
																}
																else
																{
																								dE.action=0;
																...
																if(mode=="asincrono") {
																								cout<<"Esta dentro de la parte asincrona"<<endl;
																								set< int >::iterator it;
																								it=conectados.begin();
																								for(it; it!=conectados.end(); it++) {
																																if(dc[*it].follow.find(opcion.id)!=dc[*it].follow.end())
																																{
																																								int status=*it;
																																								datosEnvio nuevo;
																																								dc[status].follow[opcion.id]++;
																																								nuevo.id=opcion.id;
																																								strcpy(nuevo.message,message.c_str());
																																								cout<<status<<" "<<pipeVector[status]<<endl;
																																								comunicadora(pipeVector[status],nuevo);
																																								kill (pids[status], SIGUSR1);
...
int main (int argc, char **argv)
{
								...
								while((aux=read(pipeG, &dRecibido, sizeof(dRecibido))!=-1))
								{
																if(aux>0) {
																								...
																								cout<<"--------------------------------------------------------------------------------"<<endl;
																								cout<<"Esperando nueva opcion desde cliente...\n";
																}
																sleep(2);
